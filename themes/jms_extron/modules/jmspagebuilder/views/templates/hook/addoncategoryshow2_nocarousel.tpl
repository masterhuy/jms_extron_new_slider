...
<script type="text/javascript">
	var cs_no_items = {if $cols}{$cols|escape:'htmlall':'UTF-8'}{else}4{/if};
	var cs_no_itemsDesktop = {if $cols}{$cols|escape:'htmlall':'UTF-8'}{else}4{/if};
	var cs_no_itemsDesktopSmall = {if $cols_md}{$cols_md|escape:'htmlall':'UTF-8'}{else}3{/if};
	var cs_no_itemsTablet = {if $cols_sm}{$cols_sm|escape:'htmlall':'UTF-8'}{else}2{/if};
	var cs_no_itemsMobile = {if $cols_xs}{$cols_xs|escape:'htmlall':'UTF-8'}{else}1{/if};
	var cs_no_nav = {if $navigation == 1}true{else}false{/if};
	var cs_no_pag = {if $pagination == 1}true{else}false{/if};
	var cs_no_auto_play_carousel = {if $autoplay == 1}true{else}false{/if};
</script>
{if $addon_title}
		            	...
		            	{foreach from=$category item=c_item }
							...
							<div class="categories-wrapper">
								...
								<div class="c_item-info">
									<a class="cat-name-no-cs" href="{$categoryLink nofilter}">{$c_item.name nofilter}</a>
